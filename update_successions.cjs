...
const globalGraphPath = 'src/GlobalGraph.tsx';
let content = fs.readFileSync(globalGraphPath, 'utf-8');

const newSuccessions = `const successions: Record<string, string[]> = {
  // ===== 西方哲學完整鏈 =====
...
const regex = /const successions: Record<string, string\[\]> = \{[\s\S]*?\n\};/;
const newContent = content.replace(regex, newSuccessions);

fs.writeFileSync(globalGraphPath, newContent);
console.log('Updated GlobalGraph.tsx with new successions.');
