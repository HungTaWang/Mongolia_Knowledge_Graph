...
const MODERN_DATA_PATH = 'scratch/modern_data.json';

function run() {
    let dataJson = JSON.parse(fs.readFileSync(DATA_JSON_PATH, 'utf8'));
    let newData = JSON.parse(fs.readFileSync(MODERN_DATA_PATH, 'utf8'));
    ...
    console.log(`Added ${newData.newCities.length} cities, ${newData.newRoutes.length} routes, ${newData.newTours.length} tours, and updated glossary for modern era.`);
}
